...
namespace h5 {
  ...
  void h5_write(group g, std::string const &name, std::vector<std::string> const &V) {
    ...
    if (err < 0) H5_ERROR << "Error writing the vector<string> " << name << " in the group" << g.name();
  }
  ...
  void h5_read(group g, std::string const &name, std::vector<std::string> &V) {
    ...
    if (ndims != 1) H5_ERROR << "triqs::h5 : Trying to read 1d array/vector . Rank mismatch : the array stored in the hdf5 file has rank = " << ndims;
    ...
    if (err < 0) H5_ERROR << "Error reading the vector<string> " << name << " in the group" << g.name();
  ...
  void h5_read_attribute(hid_t id, std::string const &name, std::vector<std::vector<std::string>> &V) {
    ...
    int ndims = H5Sget_simple_extent_dims(d_space, dims_out.data(), NULL);
    if (ndims != 2)
      H5_ERROR << "triqs::h5 : Trying to read vector<vector<string>>. Rank mismatch : the array stored in the hdf5 file has rank = " << ndims;
    ...
    if (s_size * dims_out[0] * dims_out[1] != size) H5_ERROR << "Error in rereading the attribute. Size mismatch of the array";
    V.clear();
