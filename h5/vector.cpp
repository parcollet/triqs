...
namespace h5 {
  ...
  namespace {
    ...
    dataspace dataspace_from_LS(int rank, hsize_t const *L, hsize_t const *S) {
      ...
      if (!ds.is_valid()) throw make_std_runtime_error("Cannot create the dataset");

      herr_t err = H5Sselect_hyperslab(ds, H5S_SELECT_SET, offsetf, stridesf, dimsf, NULL);
      if (err < 0) throw make_std_runtime_error("Cannot set hyperslab");
  ...
  void h5_write(group g, std::string const &name, std::vector<std::string> const &V) {
    ...
    if (err < 0) throw make_std_runtime_error("Error writing the vector<string> ", name, " in the group", g.name());
  }
  ...
  void h5_write_attribute(hid_t id, std::string const &name, std::vector<std::string> const &V) {
    ...
    if (!attr.is_valid()) throw make_std_runtime_error("Cannot create the attribute ", name);

    herr_t status = H5Awrite(attr, aux.strdatatype, (void *)aux.buf.data());
    if (status < 0) throw make_std_runtime_error("Cannot write the attribute ", name);
  }
  ...
  void h5_read(group g, std::string const &name, std::vector<std::string> &V) {
    ...
    if (ndims != 1)
      throw make_std_runtime_error("triqs::h5 : Trying to read 1d array/vector . Rank mismatch : the array stored in the hdf5 file has rank = ",
                                   ndims);
    ...
    if (err < 0) throw make_std_runtime_error("Error reading the vector<string> ", name, " in the group", g.name());
  ...
  void h5_read_attribute(hid_t id, std::string const &name, std::vector<std::vector<std::string>> &V) {
    ...
    if (!attr.is_valid()) throw make_std_runtime_error("Cannot open the attribute ", name);
    ...
    int ndims = H5Sget_simple_extent_dims(d_space, dims_out.data(), NULL);
    if (ndims != 2)
      throw make_std_runtime_error("triqs::h5 : Trying to read vector<vector<string>>. Rank mismatch : the array stored in the hdf5 file has rank = ",
                                   ndims);
    ...
    if (err < 0) throw make_std_runtime_error("Cannot read the attribute ", name);

    // for (auto x : buf) std::cout << "bufr "<<  int(x) << std::endl;
    int s_size = size / dims_out[0] / dims_out[1]; // size of the string
    if (s_size * dims_out[0] * dims_out[1] != size) throw make_std_runtime_error("Error in rereading the attribute. Size mismatch of the array");
    V.clear();
