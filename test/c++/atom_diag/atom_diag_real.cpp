...
TEST(atom_diag_real, Autopartition) {
  ...
  EXPECT_EQ(fock_states_ref, fock_states);
#endif
  ...
  for (int sp : range(ad.n_subspaces())) {
    ...
    EXPECT_ARRAY_NEAR(umat_ref[sp] * e_diag * transpose(umat_ref[sp]), umat[sp] * e_diag * transpose(umat[sp]));
  ...
  std::vector<std::vector<matrix<double>>> c_matrices(fops.size(), std::vector<matrix<double>>(ad.n_subspaces()));
  std::vector<std::vector<matrix<double>>> cdag_matrices(fops.size(), std::vector<matrix<double>>(ad.n_subspaces()));
...
#ifndef GENERATE_REF_H5
        EXPECT_ARRAY_NEAR(umat_ref[c_conn] * c_matrices_ref[op][sp] * transpose(umat_ref[sp]),
                          umat[c_conn] * c_matrices[op][sp] * transpose(umat[sp]));
#endif
      }
...
#ifndef GENERATE_REF_H5
        EXPECT_ARRAY_NEAR(umat_ref[cdag_conn] * cdag_matrices_ref[op][sp] * transpose(umat_ref[sp]),
                          umat[cdag_conn] * cdag_matrices[op][sp] * transpose(umat[sp]));
#endif
      }
    }
  }
...
TEST(atom_diag_real, Functions) {
  ...
  // Static observables
  double N_up_obs = trace_rho_op(rho, N_up, ad);
  double N_dn_obs = trace_rho_op(rho, N_dn, ad);
  double N_obs    = trace_rho_op(rho, N, ad);
  double N2_obs   = trace_rho_op(rho, N * N, ad);
#ifndef GENERATE_REF_H5
