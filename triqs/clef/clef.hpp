...
namespace triqs {
  ...
  *  --------------------------------------------------------------------------------------------------- */
    template <typename Tag, typename... T> struct expr {
      ...
      template <typename Args> expr<tags::subscript, expr, expr_storage_t<Args>> operator[](Args &&args) const {
        return {tags::subscript(), *this, std::forward<Args>(args)};
      }
      // () also ...
      template <typename... Args> expr<tags::function, expr, expr_storage_t<Args>...> operator()(Args &&... args) const {
  ...
  *  --------------------------------------------------------------------------------------------------- */
    ...
    // Terminal
    template <> struct operation<tags::terminal> {
      template <typename L> FORCEINLINE L operator()(L &&l) const { return std::forward<L>(l); }
    };

    // Function call
    template <> struct operation<tags::function> {
      template <typename F, typename... Args> FORCEINLINE decltype(auto) operator()(F &&f, Args &&... args) const {
...
#define TRIQS_CLEF_OPERATION(TAG, OP)                                                                                                                \
  ...
  template <> struct operation<tags::TAG> {                                                                                                          \
    template <typename L> FORCEINLINE decltype(auto) operator()(L &&l) const { return OP _cl(std::forward<L>(l)); }                                  \
  };
...
#undef TRIQS_CLEF_OPERATION
    ...
    template <> struct operation<tags::if_else> {
      ...
      template <typename C, typename A, typename B> FORCEINLINE A operator()(C const &c, A const &a, B const &b) const {
 ...
 *  --------------------------------------------------------------------------------------------------- */
  ...
  *  --------------------------------------------------------------------------------------------------- */
    ...
    template <ull_t x, int I0, int... Is> struct ph_filter<x, I0, Is...> {
    ...
    template <typename Expr, int... Is> struct ph_set<make_fun_impl<Expr, Is...>> {
    ...
    template <typename Expr, int... N> auto operator>>(ph_list<N...> &&, Expr const &ex) -> decltype(make_function(ex, _ph<N>()...)) {
  ...
  * --------------------------------------------------------------------------------------------------- */
    ...
    template <int N, typename Expr> auto operator>>(_ph<N> p, Expr &&ex) { return make_function(ex, _ph<N>{}); }
  ...
  *  --------------------------------------------------------------------------------------------------- */
    ...
    template <typename T, typename F> FORCEINLINE void triqs_clef_auto_assign(std::reference_wrapper<T> R, F &&f) {
      triqs_clef_auto_assign(R.get(), std::forward<F>(f));
    }
    template <typename T, typename F> FORCEINLINE void triqs_clef_auto_assign(expr<tags::terminal, T> const &t, F &&f) {
    ...
    template <typename Tag, typename... Childs, typename RHS> FORCEINLINE void triqs_clef_auto_assign(expr<Tag, Childs...> &&ex, RHS const &rhs) {
      ex << rhs;
    }

    template <typename Tag, typename... Childs, typename RHS>
    FORCEINLINE void triqs_clef_auto_assign(expr<Tag, Childs...> const &ex, RHS const &rhs) {
      ex << rhs;
    }
...
*/
  ...
  *  --------------------------------------------------------------------------------------------------- */
    ...
    // by default it is deleted = not implemented : every class has to define it...
    template <typename T, typename F> FORCEINLINE void triqs_clef_auto_assign_subscript(T, F) = delete;

    // remove the ref_wrapper, terminal ...
    template <typename T, typename F> FORCEINLINE void triqs_clef_auto_assign_subscript(std::reference_wrapper<T> R, F &&f) {
      triqs_clef_auto_assign_subscript(R.get(), std::forward<F>(f));
    }
    template <typename T, typename F> FORCEINLINE void triqs_clef_auto_assign_subscript(expr<tags::terminal, T> const &t, F &&f) {
      triqs_clef_auto_assign_subscript(std::get<0>(t.childs), std::forward<F>(f));
    }

    // auto assign of an expr ? (for chain calls) : just reuse the same operator
    template <typename Tag, typename... Childs, typename RHS>
    FORCEINLINE void triqs_clef_auto_assign_subscript(expr<Tag, Childs...> &&ex, RHS const &rhs) {
      ex << rhs;
    }

    template <typename Tag, typename... Childs, typename RHS>
    FORCEINLINE void triqs_clef_auto_assign_subscript(expr<Tag, Childs...> const &ex, RHS const &rhs) {
      ex << rhs;
    }
  ...
  * --------------------------------------------------------------------------------------------------- */
    ...
    template <typename Obj, typename... Args> typename _result_of::make_expr_call<Obj, Args...>::type make_expr_call(Obj &&obj, Args &&... args) {
  ...
  * --------------------------------------------------------------------------------------------------- */
    ...
    template <typename Obj, typename Arg> typename _result_of::make_expr_subscript<Obj, Arg>::type make_expr_subscript(Obj &&obj, Arg &&arg) {
...
#define TRIQS_CLEF_IMPLEMENT_LAZY_CALL(...)                                                                                                          \
  template <typename... Args> auto operator()(Args &&... args) const &DECL_AND_RETURN(make_expr_call(*this, std::forward<Args>(args)...));           \
                                                                                                                                                     \
  template <typename... Args> auto operator()(Args &&... args) & DECL_AND_RETURN(make_expr_call(*this, std::forward<Args>(args)...));                \
                                                                                                                                                     \
  template <typename... Args> auto operator()(Args &&... args) && DECL_AND_RETURN(make_expr_call(std::move(*this), std::forward<Args>(args)...));
