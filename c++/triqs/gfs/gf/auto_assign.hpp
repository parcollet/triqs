...
namespace triqs {
  ...
  *-----------------------------------------------------------------------------------------------------*/
    ...
    template <typename RHS, typename M, typename T> FORCEINLINE void clef_auto_assign(gf_view<M, T> g, RHS const &rhs) {
      clef_auto_assign_impl(g, rhs, mesh::is_product<M>{});
    }

    template <typename RHS, typename M, typename T> FORCEINLINE void clef_auto_assign(gf<M, T> &g, RHS const &rhs) {
      clef_auto_assign(g(), rhs);
    }

    // enable the writing g[om_] << .... also
    template <typename RHS, typename M, typename T> FORCEINLINE void clef_auto_assign_subscript(gf_view<M, T> g, RHS const &rhs) {
      clef_auto_assign(g, rhs);
    }

    template <typename RHS, typename M, typename T> FORCEINLINE void clef_auto_assign_subscript(gf<M, T> &g, RHS const &rhs) {
      clef_auto_assign(g(), rhs);
    }
    ...
    template <typename G, typename RHS> FORCEINLINE void clef_auto_assign_impl(G &g, RHS const &rhs, std::false_type) {
      for (auto const &w : g.mesh()) { triqs_gf_clef_auto_assign_impl_aux_assign(g[w], rhs(w)); }
    }

    template <typename G, typename RHS> FORCEINLINE void clef_auto_assign_impl(G &g, RHS const &rhs, std::true_type) {
